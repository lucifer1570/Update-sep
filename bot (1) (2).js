...
function updateCombinedAfterResult(userId, sizeWon, numberWon, betPlaced) {
    ...
    if (sizeWon || numberWon) {
        st.sizeLevel = 1; st.numberLevel = 1; st.level = 1; st.consecutiveLoss = 0; st.lossStreakHitRecorded = false;
    } else {
        st.consecutiveLoss++;
...
function getPredictionSelection(lastResult, historyResults) {
    ...
    const candidates = predictionMap[n][1].map(Number);
    let selectedNumber = candidates[0];
    let selectedIndex = Infinity;

    for (const candidate of candidates) {
        const index = Array.isArray(historyResults)
            ? historyResults.findIndex((item, i) =>
                i > 0 && Number(item.number) === candidate
            )
            : -1;
    ...
    return {
        mapping: [predictionMap[n][0], selectedNumber],
        mode: 'MOST_RECENT',
        candidates,
        matchedApiIndex: selectedIndex === Infinity ? null : selectedIndex,
        decisionReason: selectedIndex === Infinity
            ? 'No mapped candidate found; selected first candidate'
            : 'Selected most-recent mapped candidate'
    };
}
...
async function handleWin(userId, chatId, actual, num, betLevel, bets = [], settlement = null) {
    ...
    let profit;
    profit = settlement ? Number(settlement.pnl) || 0 : amt * 0.98;
